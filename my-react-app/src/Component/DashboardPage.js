...
import Navbar from './Navbar'; // Pastikan di-import

function DashboardPage() {
    const [user, setUser] = useState(null);
    const navigate = useNavigate();

    // 1. Logika Verifikasi Token & Pengalihan
    useEffect(() => {
        const token = localStorage.getItem('token');
        if (!token) {
            navigate('/login'); // Jika TIDAK ADA token, paksa ke Login
            return;
        }
        try {
            const decoded = jwtDecode(token);
            // Cek jika token sudah kedaluwarsa (exp adalah timestamp dalam detik)
            if (decoded.exp * 1000 < Date.now()) {
                localStorage.removeItem('token');
                navigate('/login');
            } else {
                setUser(decoded); // Set user data (id, nama, role)
            }
        } catch (error) {
    ...
    // 2. Tampilkan "Memuat" saat data user belum selesai diambil
    if (!user) {
        // Tampilkan Navbar untuk menghindari blank screen total, tetapi konten diisi 'Memuat'
        return (
            <>
                <Navbar /> 
